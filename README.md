...
작업 폴더는 `$DOBBY_WORKSPACE/subtree/{repo}-{이슈키}`, 메타는 `$DOBBY_META/.issue-start/...`(기본 `$DOBBY_WORKSPACE/meta`, `DOBBY_META_PATH`로 별도 지정 가능)처럼 **분리 저장**됩니다. 설정 예시는 `plugins/work-dobby/config.env.example` 참고.

## 설치

```
/plugin marketplace add <github-owner>/mentis-plugins
/plugin install work-dobby@mentis-plugins
```
...
└── plugins/work-dobby/
    ...
    └── skills/
        ...
        └── agent-start/  agent-impl/  i-order-you-to-develop/
```
