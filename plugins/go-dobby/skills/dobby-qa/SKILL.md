...
4. **상태 선(先)갱신**(P8 규칙): 작업 전에 `orchestration.md` 상태표와 `status.md` `## 에이전트` 표를 **둘 다** 해당 에이전트 `완료 → 수정중`(재분석 필요하면 `분석중`), **라운드 +1**, 착수 시각·갱신 일시 기록. `## 이벤트 로그`에 `- {YYYY-MM-DD HH:MM} QA: {슬러그} 버그 {버그키} 편입(기준 ①/②/③) — {요약}` 1줄.
...
- blocking(blocker·major)=0이 될 때까지 `리뷰중`↔`수정중` 반복(**최대 라운드 기본 3**, 초과 시 사용자 에스컬레이션).

### Q6. 재통합 → 검증 → 해결
- 리뷰 클린 → 담당 에이전트 브랜치를 **루트 브랜치로 재머지·푸시**한다(에이전트가 하나면 그 브랜치가 곧 루트라 머지 없음). ⛔ **정식 배포 베이스(master)로의 PR·머지는 금지**. 상태 `재통합대기 → 완료`.
