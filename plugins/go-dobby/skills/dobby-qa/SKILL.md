...
4. **상태 선(先)갱신**(P8 규칙): 작업 전에 `orchestration.md` 상태표와 `status.md` `## 에이전트` 표를 **둘 다** 해당 에이전트 `완료 → 구현`(재분석 필요하면 `분석`), **라운드 +1**, 착수 시각·갱신 일시 기록. `## 이벤트 로그`에 `- {YYYY-MM-DD HH:MM} QA: {슬러그} 버그 {버그키} 편입(기준 ①/②/③) — {요약}` 1줄.
