...
- 커밋·푸시는 하지 않는다.
- **status.md 초기화(단일/루트만)**: 에이전트키 = 루트키면 `$ORCHESTRATION_META/{루트키}/status.md`에 이슈/작업 메타·현재 단계 `착수`·워크트리/브랜치·베이스·시작 일시를 기록한다(스키마는 아래). **fan-out 하위(에이전트키 ≠ 루트키)면 status.md를 만들지 않는다**(단계·상태는 루트 orchestration.md 상태표 — 아래 "메타는 루트 폴더 하나" 규칙).
