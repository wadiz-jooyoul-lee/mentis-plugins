...
### 5. 분석 (코드 위치까지)
- **⛔ 착수 시 docs 빠른 키워드 검색(기본 · 코드 분석 전 먼저)**: 코드를 뒤지기 전에, 이슈 제목·핵심 키워드(기능명·컴포넌트·화면·URL·도메인 용어)로 **`$ORCHESTRATION_DOCS_ROOT`를 grep해 관련 문서 후보를 먼저 찾는다**. 예: `grep -rilE '{키워드1}|{키워드2}' "$ORCHESTRATION_DOCS_ROOT" | head`. 히트가 있으면 **관련된 문서만 열어** 배경·플로우·용어·계약을 파악한 뒤 코드 분석에 들어간다.
