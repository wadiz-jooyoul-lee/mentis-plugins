...
**하나의 규칙(K로 분기하지 않음):** 루트 브랜치 `{prefix}/{루트키}`를 base에서 만든다(통합 지점). 에이전트키 ≠ 루트키인 에이전트는 `{prefix}/{에이전트키}`를 루트 브랜치에서 만들어 작업하고 리뷰 후 **루트 브랜치로 머지**한다.

- 에이전트가 하나뿐이면 그 에이전트키 = 루트키 → 루트 브랜치가 곧 에이전트 브랜치. 추가 브랜치·통합 머지가 자연히 없다(별도 부모 브랜치 안 만듦). 이는 "에이전트 수 1"의 귀결일 뿐 특수 분기가 아니다.
- `{base} └ {prefix}/{루트키} [├ {prefix}/{에이전트A} └ {prefix}/{에이전트B} …]` (대괄호는 에이전트가 여럿일 때).
