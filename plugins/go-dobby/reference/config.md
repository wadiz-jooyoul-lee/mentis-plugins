...
| `dobby-meta-backup.sh KEY \| --all \| --inprogress [am\|pm]` | 해결 시(자동) · 사용자(일괄) · 대시보드(진행중) | 메타 폴더 텍스트를 `{폴더이름}--{시각}.tar.zst`로 압축. 이미지 제외, 검증 통과 후 이전 아카이브 교체. `--all`은 변경된 폴더만. `--inprogress`는 해결·종료가 아닌 폴더를 한 덩이로 모아 `inprogress/inprogress-{날짜}-{am\|pm}.tar.zst`로 임시 보관(보관기간 지난 것 삭제) |
