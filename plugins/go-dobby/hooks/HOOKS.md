...
## G14·G15·G16 — 회고 기반 게이트 (2026-09-07)

회고 4건(FE1-1732·FE1-1809·FE1-1803·FE1-1808)의 개선 제안을 "판단 없이 감지 가능한가"(위 절차 1번)로
가른 결과다. **판단이 필요한 제안은 전부 스킬 문서 몫으로 남겼고**, 아래 셋만 코드로 옮겼다.

### 왜 훅이 아니라 헬퍼(코드 강제)인가
셋 다 파일 존재·숫자 비교라 훅으로도 만들 수 있지만, `dobby_lint`는 이미 `dobby_gate`가 단계
전이마다 호출하므로 **새 훅 스크립트 등록이 필요 없다**(플러그인 훅 전달 버그 #34573 우회 불필요).
G16은 워크트리 생성 시점에만 필요해 그 헬퍼 안이 정확한 자리다.

### 셋 다 경고(`_w`)이고 치명(`_e`)이 아닌 이유
109개 오더 전량에 돌려 확인했다 — **치명 건수 23건 → 23건(변화 0), 경고 74건 → 89건(+15).**
즉 `dobby_lint strict`를 쓰는 `dobby_gate`의 전이 판정에 영향이 없다. 규율 6("확신이 없으면
차단하지 말고 통과")에 따라 적발 정확도가 검증되기 전에는 차단하지 않는다.

### 실측 근거

| 규칙 | 발화 | 조건을 좁힌 이유 |
|---|---|---|
| G14 | 5/71건(구현 롤 있는 오더 기준) | 조건 없이 검사하면 29/71건(41%)이고 대부분이 인라인 오더의 해소 불가 경고다. `agents/`에 계약이 1개 이상인 오더로 한정해 5건으로 줄였고 전부 진짜 누락이다(FE1-1317·1320·1423·1531·1732) |
| G15 | 10/71건 | 라운드 4~10까지 간 오더 **전부** 에스컬레이션 기록이 0건이었다. 오탐 여지가 없다 |
| G16 | 22개 경로·572바이트·0.12초 | `CLAUDE.md`는 깊이 무제한(깊이 3·4로 자르면 18개 중 15개만 잡히고, 놓치는 것이 하필 수정 대상 폴더다). `README.md`는 상위 2단계까지만(깊이를 풀면 63개로 불어난다) |

### 채택하지 않은 것 — "라운드 수 vs reviews/ 폴더 수" 검사
회고 FE1-1732 제안 6이 요구했으나 **오탐률이 허용 범위를 넘어 넣지 않았다.** 세 변형을 109개
오더에 돌린 결과: 느슨(리뷰 파일 0개)은 0건으로 무용, 중간(최대라운드 > 폴더 수)은 13건이지만
라운드 값이 누적 카운터라 폴더 수와 일치할 근거가 없고, 엄격(슬러그명 findings 파일 필요)은
6건 중 3건이 오탐이었다.

원인은 규약과 현실의 불일치다. dobby-order P5는 findings 파일명을 "지적이 향하는 담당 에이전트
슬러그"로 규정하는데, **실제로는 리뷰어 이름(`review-agent.md`)이나 영역명(`global.md`)으로
저장하는 관행이 현행에서 살아 있다**(FE1-1814는 2026-09-07 오더인데 그 형태다). 파일명 규약을
먼저 정리해야 이 검사가 성립한다. 그때까지는 G14가 "계약 누락"이라는 인접 신호로 대신 잡는다.
