...
- 삭제 대상 = (이슈가 **종료**) AND (미커밋 변경 없음) AND (미푸시 커밋 없음).
- 처리 전에 **표로 요약**해 보여준다: 폴더 / 이슈 상태 / dirty·unpushed 여부 / 처리(삭제·유지).
  - 종료지만 미커밋·미푸시가 있는 폴더는 **유지**하고 사유를 명확히 표시한다(사용자가 따로 정리하도록).
  - 진행 중 이슈는 유지.
...
- 삭제 대상 각 폴더를 소속 원본 repo에서 정식 제거한다: `git -C {원본repo} worktree remove {폴더 절대경로}`
  ...
  - `--force`는 쓰지 않는다(미커밋 변경이 있으면 애초에 4단계에서 유지 처리됨).
...
- 종료됐지만 미커밋·미푸시로 워크트리를 유지한 경우, 처리를 "유지"로 적고 사유를 남긴다(서머리는 그대로 저장).
